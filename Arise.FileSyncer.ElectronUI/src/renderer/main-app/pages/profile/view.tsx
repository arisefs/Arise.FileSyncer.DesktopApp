...
import { DeleteProfileResult } from "../../../../main/ipc/messages";
...
export class ViewProfilePage extends React.Component<ViewProfilePageProps, ViewProfilePageState> {
    ...
    public componentDidMount() {
        ...
        ipcRenderer.addListener("srvDeleteProfileResult", this.onDeleteResult);
    }

    public componentWillUnmount() {
        this.profilesEvent.dispose();

        ipcRenderer.removeListener("srvDeleteProfileResult", this.onDeleteResult);
    }
    ...
    private onProfilesChanged(sd: Shared.Dictionary<Interfaces.Profile>) {
        let profile = sd.get(this.props.id);
    ...
    private onEdit() {
        let props: EditProfilePageProps = {
            id: this.props.id
        }
    ...
    private onShare() {
        let props: ShareProfilePageProps = {
    ...
    private onDelete() {
        ipcRenderer.send("delete-profile", this.props.id);
    }

    private onDeleteResult(_e: any, message: DeleteProfileResult.Message) {
        if (message.Success) {
            pushNotification({
                type: "success",
