...
export interface ProfileEditorProps {
    ...
    onDirectorySelected: (filePaths: string[], bookmarks: string[]) => void,
}
...
export function handleTextBoxChange(event: React.ChangeEvent<HTMLInputElement>) {
    this.setState({ [event.target.name]: event.target.value });
}

export function handleCheckBoxChange(name: string) {
    this.setState({ [name]: !this.state[name] });
}

export function handleDirectorySelected(filePaths: string[], _bookmarks: string[]) {
    if (filePaths.length > 0) {
        this.setState({ rootDir: filePaths[0] });
    }
}
